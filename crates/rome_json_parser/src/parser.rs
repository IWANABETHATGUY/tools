...
impl ParserProgress {
    ...
    #[inline]
    pub fn has_progressed(&self, p: &Parser<'_>) -> bool {
        match self.0 {
            None => true,
...
impl<'s> Parser<'s> {
    ...
    #[inline]
    pub fn at(&self, kind: JsonSyntaxKind) -> bool {
        self.cur() == kind
    }
    ...
    // #[inline]
    // pub fn nth_at(&mut self, n: usize, kind: JsonSyntaxKind) -> bool {
    //     self.nth(n) == kind
    // }

    /// Returns the kind of the last bumped token.
    pub fn last(&self) -> Option<JsonSyntaxKind> {
    ...
    #[inline]
    pub fn eat(&mut self, kind: JsonSyntaxKind) -> bool {
        if !self.at(kind) {
            return false;
        }
    ...
    #[inline]
    pub fn bump(&mut self, kind: JsonSyntaxKind) {
        assert_eq!(
            kind,
            self.cur(),
            "expected {:?} but at {:?}",
    ...
    pub fn bump_remap(&mut self, kind: JsonSyntaxKind) {
        self.do_bump(kind)
    }
    ...
    pub fn bump_any(&mut self) {
        let kind = self.cur();
        assert_ne!(kind, JsonSyntaxKind::EOF);
        self.do_bump(kind)
    }
    ...
    fn do_bump(&mut self, kind: JsonSyntaxKind) {
    ...
    pub fn expect(&mut self, kind: JsonSyntaxKind) -> bool {
        if self.eat(kind) {
            true
        } else {
...
impl Marker {
    ...
    pub fn complete(mut self, p: &mut Parser, kind: JsonSyntaxKind) -> CompletedMarker {
        self.bomb.defuse();
...
impl CompletedMarker {
    pub fn new(start_pos: u32, finish_pos: u32, offset: TextSize, kind: JsonSyntaxKind) -> Self {
        CompletedMarker {
            start_pos,
            offset,
            old_start: start_pos,
    ...
    pub fn change_kind(&mut self, p: &mut Parser, new_kind: JsonSyntaxKind) {
        self.kind = new_kind;
        match p
            .events
...
// mod tests {
//     use crate::Parser;
//     use rome_js_syntax::{JsonSyntaxKind, SourceType};
...
//         let m = p.start();
//         p.expect(JsonSyntaxKind::JS_STRING_LITERAL);
//         m.complete(&mut p, JsonSyntaxKind::JS_STRING_LITERAL_EXPRESSION);
//     }
